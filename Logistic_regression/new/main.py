...
for i in range(len(l)):
    ...
    fft_train=np.nan_to_num(fft_train)
     ...
     #Logistic Regression
    ansx = module.logistic_regression(fft_train,fft_test,np_train_label,np_test_label)
    print(f'{i}:{1-ansx}')
    ans.append(float("{0:.4f}".format(1-ansx)))
result =   {
    "Name" : Name,
    "Logistic Regression(error)"  : ans
}   
d = pd.DataFrame(result)
d.to_csv('result_logistic_regression(real_imag_phase_amp).csv')