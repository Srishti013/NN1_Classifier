...
def real_imag_phase_amp(data):
    data =  np.concatenate((np.sqrt((data.real)**2 + (data.imag)**2), np.angle(data) , data.real,data.imag), axis=1)
    print(data.shape)
    return data
...
def logistic_regression(np_train,np_test,np_train_label,np_test_label):
    model=LogisticRegression(solver='newton-cg',max_iter=math.inf)
